...
def generate_dirty_data(
    ...
    type_mismatch_count = max(1, int(round(base_count * 0.05)))
    mismatch_idx = _choose_available_indices(np.flatnonzero(remaining), type_mismatch_count, rng)
    ...
    malformed_date_count = max(1, int(round(base_count * 0.05)))
    malformed_idx = _choose_available_indices(np.flatnonzero(remaining), malformed_date_count, rng)
    invalid_dates = [
        "2026-13-40 25:61:00",
        "not_a_date",
        "2026/99/01",
    ...
    drift_count = max(1, int(round(base_count * 0.05)))
